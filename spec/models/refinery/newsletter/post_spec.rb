...
module Refinery
  module Newsletter
    describe Post do
      let(:post) { FactoryGirl.create(:newsletter_post) }

      describe "validations" do
        it "requires title" do
          FactoryGirl.build(:newsletter_post, :title => "").should_not be_valid
        end

        it "won't allow duplicate titles" do
          FactoryGirl.build(:newsletter_post, :title => post.title).should_not be_valid
        end

        it "requires body" do
          FactoryGirl.build(:newsletter_post, :body => nil).should_not be_valid
        end
      end
      ...
      describe "comments association" do
        ...
        it "destroys associated comments" do
          FactoryGirl.create(:newsletter_comment, :newsletter_post_id => post.id)
          post.destroy
          Newsletter::Comment.where(:newsletter_post_id => post.id).should be_empty
        end
      end
      ...
      describe "by_archive" do
        before do
          @post1 = FactoryGirl.create(:newsletter_post, :published_at => Date.new(2011, 3, 11))
          @post2 = FactoryGirl.create(:newsletter_post, :published_at => Date.new(2011, 3, 12))

          #2 months before
          FactoryGirl.create(:newsletter_post, :published_at => Date.new(2011, 1, 10))
        end
      ...
      describe ".published_dates_older_than" do
        before do
          @post1 = FactoryGirl.create(:newsletter_post, :published_at => Time.now - 2.months)
          @post2 = FactoryGirl.create(:newsletter_post, :published_at => Time.now - 1.month)
          FactoryGirl.create(:newsletter_post, :published_at => Time.now)
        end
      ...
      describe "live" do
        before do
          @post1 = FactoryGirl.create(:newsletter_post, :published_at => Time.now.advance(:minutes => -2))
          @post2 = FactoryGirl.create(:newsletter_post, :published_at => Time.now.advance(:minutes => -1))
          FactoryGirl.create(:newsletter_post, :draft => true)
          FactoryGirl.create(:newsletter_post, :published_at => Time.now + 1.minute)
        end
      ...
      describe "uncategorized" do
        before do
          @uncategorized_post = FactoryGirl.create(:newsletter_post)
          @categorized_post = FactoryGirl.create(:newsletter_post)

          @categorized_post.categories << FactoryGirl.create(:newsletter_category)
        end
      ...
      describe "#live?" do
        it "returns true if post is not in draft and it's published" do
          Factory.build(:newsletter_post).should be_live
        end

        it "returns false if post is in draft" do
          Factory.build(:newsletter_post, :draft => true).should_not be_live
        end

        it "returns false if post pub date is in future" do
          Factory.build(:newsletter_post, :published_at => Time.now.advance(:minutes => 1)).should_not be_live
        end
      end

      describe "#next" do
        before do
          FactoryGirl.create(:newsletter_post, :published_at => Time.now.advance(:minutes => -1))
      ...
      describe "#prev" do
        before do
          FactoryGirl.create(:newsletter_post)
          @post = FactoryGirl.create(:newsletter_post, :published_at => Time.now.advance(:minutes => -1))
        end
      ...
      describe ".comments_allowed?" do
        context "with Refinery::Setting comments_allowed set to true" do
          before do
            Refinery::Setting.set(:comments_allowed, { :scoping => 'newsletter', :value => true })
          end
        ...
        context "with Refinery::Setting comments_allowed set to false" do
          before do
            Refinery::Setting.set(:comments_allowed, { :scoping => 'newsletter', :value => false })
          end
      ...
      describe "custom teasers" do
        it "should allow a custom teaser" do
          FactoryGirl.create(:newsletter_post, :custom_teaser => 'This is some custom content').should be_valid
        end
      end
      ...
      describe ".teasers_enabled?" do
        context "with Refinery::Setting teasers_enabled set to true" do
          before do
            Refinery::Setting.set(:teasers_enabled, { :scoping => 'newsletter', :value => true })
          end
        ...
        context "with Refinery::Setting teasers_enabled set to false" do
          before do
            Refinery::Setting.set(:teasers_enabled, { :scoping => 'newsletter', :value => false })
          end
      ...
      describe "source url" do
        it "should allow a source url and title" do
          p = FactoryGirl.create(:newsletter_post, :source_url => 'google.com', :source_url_title => 'author')
          p.should be_valid
      ...
      describe ".validate_source_url?" do
        context "with Refinery::Newsletter.validate_source_url set to true" do
          before do
            Refinery::Newsletter.validate_source_url = true
          end  
          it "should have canonical url" do
            p = FactoryGirl.create(:newsletter_post, :source_url => 'google.com', :source_url_title => 'google')
            p.source_url.should include('www')
          end
        end
        context "with Refinery::Newsletter.validate_source_url set to false" do
          before do
            Refinery::Newsletter.validate_source_url = false
          end
