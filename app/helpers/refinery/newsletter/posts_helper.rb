module Refinery
  module Newsletter
    module PostsHelper
      ...
      def newsletter_post_teaser_enabled?
        Refinery::Newsletter::Post.teasers_enabled?
      end

      def newsletter_post_teaser(post)
        if post.respond_to?(:custom_teaser) && post.custom_teaser.present?
         post.custom_teaser.html_safe
        else
         truncate(post.body, {
           :length => Refinery::Newsletter.post_teaser_length,
           :preserve_html_tags => true
          }).html_safe
        end
      end

      def newsletter_archive_widget(dates=newsletter_archive_dates)
        ArchiveWidget.new(dates, self).display
      end

      def newsletter_archive_dates(cutoff=Time.now.beginning_of_month)
        Refinery::Newsletter::Post.published_dates_older_than(cutoff)
      end
      ...
      class ArchiveWidget
        ...
        def recent_link(year, month, count)
          link_to "#{t("date.month_names")[month]} #{year} (#{count})",
            refinery.newsletter_archive_posts_path(:year => year, :month => month)
        end
        ...
        def old_link(year, count)
          link_to "#{year} (#{count})", refinery.newsletter_archive_posts_path(:year => year)
        end
        ...
        def display
          ...
          render "refinery/newsletter/widgets/newsletter_archive", :links => links
        end
      end
    end
  end
